# Change Plan: Add `print-tape` CLI Command to `bub_sf.hook` (v2)

## Facts

1. `bub_sf.hook.SFHookImpl` provides a `SQLiteForkTapeStore` (`tape_store.db`) via the `provide_tape_store` hook (`bub_sf/src/bub_sf/hook.py:203-206`).
...
10. `TapeQuery` supports `kinds` filtering and `limit` via `fetch_all` (`republic/src/republic/tape/query.py:47-51`). `BuildQuery.build()` translates these to SQL (`bub_sf/src/bub_sf/store/query.py:144-145`).
11. `read()` returns `None` if the tape does not exist (`bub_sf/src/bub_sf/store/fork_store.py:429-432`).
...
1. **`bub print-tape <tape_name>`** — Print entries of a tape.
2. **`bub list-tapes`** — Print all tape names.
...
- If `read()` returns `None` for `print_tape`: print `Tape '<name>' not found.` to stderr and exit with code 1.
- If `list_tapes()` returns an empty list: print `(no tapes)` to stdout and exit cleanly with code 0.

### Filtering Implementation

- If neither `--limit` nor `--kind` is provided, use `store.read(tape_name)` for simplicity.
- If either is provided, construct a `TapeQuery(store=store, tape=tape_name)`, apply `.kinds(*kind)` and/or `.limit(limit)`, then call `asyncio.run(store.fetch_all(query))`.
...
6. **Filtering works:** `TapeQuery.kinds()` and `.limit()` are already implemented in the store's query builder, so the CLI options are fully supported.
