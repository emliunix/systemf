...
class TestElab3SyntaxSample:
    ...
    def test_elab3_sample_program(self, elab3_syntax_sample):
        """Parse comprehensive sample and verify top-level structure only."""
        imports, result = parse_program(elab3_syntax_sample)

        assert [i.module for i in imports] == ["List"]
        assert [type(d).__name__ for d in result] == [
            "SurfaceDataDeclaration",
            "SurfaceDataDeclaration",
            "SurfaceDataDeclaration",
            "SurfaceTermDeclaration",
